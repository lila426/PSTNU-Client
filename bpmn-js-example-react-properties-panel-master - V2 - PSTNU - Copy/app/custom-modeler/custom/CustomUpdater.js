...
export default function CustomUpdater(eventBus, modeling, bpmnjs) {
  ...
  function updateCustomElement(e) {
    ...
    if (shape.id) {
        assign(businessObject,pick(shape, [ 'id']));
    }
    ...
    var   removedElements = bpmnjs._removedElements;

    // make sure element is added / removed from bpmnjs.customElements
    if (!parent) {
      collectionRemove(customElements, shape);
      collectionAdd(removedElements, shape);
    } else {
      collectionAdd(customElements, shape);
    }
  ...
  function updateCustomConnection(e) {
    ...
    // make sure element is added / removed from bpmnjs.customElements
    if (!parent) {
      collectionRemove(customElements, connection);
    } else {
      collectionAdd(customElements, connection);
    }
    ...
    if (connection.id) {
        assign(businessObject,pick(connection, [ 'id']));
    }
